    ...
    for r in rela_result:
        ...
        if r['relationType'] == '0':
            if sourceRE.isUncertain:
                if checkinscope(sourceRE.getF()['operation'],sourceRE.scope,"operation"):
                    targetRE.spread=1
                    targetRE.sourceid=sourceRE.getID()
...
def getResWithFileName(strupath:str,relapath:str):
    global SReFile,RelationFile
    SReFile=strupath
    RelationFile=relapath
    readStructedRes()
    readRelations()
    getREs(testF)
    return str(analyzeSpread(result))
